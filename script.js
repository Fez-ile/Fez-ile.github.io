...
document.addEventListener('click', (event) => {
    ...
    const isMenuClick = navMenu.contains(target) || (menuBtn && menuBtn.contains(target));
...
const typedSpan = typewriterElement.querySelector('.hero-typed');
const texts = ['Full Stack Web Developer', 'Systems Development Graduate'];
let textIndex = 0;
let charIndex = 0;
...
function typeWriter() {
    ...
    if (isDeleting) {
        typedSpan.textContent = currentText.substring(0, charIndex - 1);
        charIndex--;
    } else {
        typedSpan.textContent = currentText.substring(0, charIndex + 1);
        charIndex++;
